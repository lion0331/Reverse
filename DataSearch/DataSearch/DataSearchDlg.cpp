...
void CDataSearchDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT3, m_memshow);
...
BOOL CDataSearchDlg::OnInitDialog()
{
	...
	m_lstAddr.InsertColumn(0, _T("地址"), 0, 80);
...
#ifdef WIN32
			CString dll_path = "E:\\edoyun\\public\\DataSearch\\Debug\\inject.dll";
#else
...
void CDataSearchDlg::UpdateModule(DWORD nPID)
{
	...
	while (bMore) {
		...
		snprintf(pid, sizeof(pid), "%08llX", (UINT64)entry.hModule);//
		m_module.SetItemText(i, 1, pid);
		snprintf(pid, sizeof(pid), "%08llX", (UINT64)entry.modBaseAddr);
...
void CDataSearchDlg::OnBnClickedBtnMemSearch()
{
	...
	if (text == "内存搜索") {
		...
		GetProcessMemoryInfo(m_hProcess, &mem, sizeof(mem));
		for (auto& entry : m_lstModules) {
			TRACE("module:%s %08X %08X\r\n",
...
LRESULT CDataSearchDlg::OnProgress(WPARAM wParam, LPARAM lParam)
{
	...
	size_t nSize = (BYTE*)(SearchTool::Instance()->m_info.lpMaximumApplicationAddress) - (BYTE*)(SearchTool::Instance()->m_info.lpMinimumApplicationAddress);
	if (m_curpos >= nSize && (m_hProcess != NULL)) {
		m_btn_search.SetWindowText("内存搜索");
		SearchTool::Instance()->SetProcess(NULL);
		CloseHandle(m_hProcess);
		m_hProcess = NULL;
...
LRESULT CDataSearchDlg::OnSecond(WPARAM wParam, LPARAM lParam)
{
	...
	if (pResult) {
		...
		for (SIZE_T addr : lstAddr) {
			...
			char buffer[32];
			snprintf(buffer, sizeof(buffer), "%llX", addr);
...
#ifdef WIN64
		snprintf(line, sizeof(line), "%08llX: ", nAddress + i);
#else
